...
Public Class MainWindow
    ...
    Public Class LEDFactory
        ...
        Public Sub CreateLED(ByVal name As String)
            ...
            Select Case name
                Case "ABORT"
                    led = New LED(name, , LED.LEDType.Abort, False)
                Case "ESTOP"
                    led = New LED(name, , LED.LEDType.EStop, False)
                Case "PL_ON"
                    led = New LED(name, , LED.LEDType.PlasmaOn, False)
                Case Else
                    led = New LED(name)
            End Select
            _ledList.Add(led)
    ...
    Public Class ControlBoard
        ...
        Public displayStatus As Boolean = False
        ...
        Private Sub UpdatedLEDsFromStatusBits()
            ...
            Next
        End Sub
        Function CheckForEStopActivation() As Boolean
            For Each led As LED In LEDS.LedList
                If led.Type = LED.LEDType.EStop AndAlso led.IsOn() Then
                    ' ESTOP LED is on, take the response action
                    Return True
                End If
            Next
            Return False
        End Function
        Function CheckForAbortActivation() As Boolean
            For Each led As LED In LEDS.LedList
                If led.Type = LED.LEDType.Abort AndAlso led.IsOn() Then
                    ' Abort LED is on, take the response action
                    Return True
                End If
            Next
            Return False
        End Function
        Function CheckForPlasmaActivation() As Boolean
            For Each led As LED In LEDS.LedList
                If led.Type = LED.LEDType.PlasmaOn AndAlso led.IsOn() Then
                    ' PlasmaOn LED is on, take the response action
                    Return True
                End If
            Next
            Return False
    ...
    Structure MFController
        ...
        Public Sub SetActualFlow(ByVal flow As Double)
            db_ActualFlow = flow
        End Sub
        Public Sub SetLoadedFlow(ByVal flow As Double)
            db_LoadedFlow = flow
        End Sub
        Public Sub SetActualFlowFromString(ByVal flow As String)
            ...
            End If
        End Sub
        Public Function isNewLoadValue() As Boolean
            Return loadNewValue
        End Function
        Public Function GetActualFlow() As Double
            Return db_ActualFlow
        End Function
        Public Function GetLoadedValue() As Double
            Return db_LoadedFlow
        End Function
        Public Function GetLoadedValueAsString() As String
            Return db_LoadedFlow.ToString()
        End Function
    ...
    Dim b_PlasmaOn As Boolean = False 'the plasma is on  
    ...
    Public Enum AbortCode
        ...
        AC_NO_CDA = &HE
    End Enum
    Private AbortCodeMessages As Dictionary(Of Int32, String) = New Dictionary(Of Int32, String) From {
        {AbortCode.AC_NO_N2, "NO PURGE N2"},
        {AbortCode.AC_NO_HEARTBEAT, "NO HEARTBEAT"},
        {AbortCode.AC_NO_GAS_1, "MFC_1 Low Flow"},
        {AbortCode.AC_NO_GAS_2, "MFC_2 Low Flow"},
        {AbortCode.AC_NO_GAS_3, "MFC_3 Low Flow"},
        {AbortCode.AC_NO_GAS_4, "MFC_4 Low Flow"},
        {AbortCode.AC_NO_GAS_5, "MFC_5 Low Flow"},
        {AbortCode.AC_NO_GAS_6, "MFC_6 Low Flow"},
        {AbortCode.AC_BAD_HELIUM, "BAD HELIUM"},
        {AbortCode.AC_ESTOP, "ESTOP ACTIVE, application shutting down."},
        {AbortCode.AC_DOORS_OPEN, "ABORT: DOOR OPENED"},
        {AbortCode.AC_PWR_FWD_LOW, "Power Fwd Low"},
        {AbortCode.AC_OVER_TEMP, "Head Too Hot"},
        {AbortCode.AC_NO_CDA, "No CDA"}
    }
    ...
    Private Sub SetGUILoadedProgressNumbers(index As Integer)
        ...
        Select Case index
            ...
            Case 4
                ...
                stringFormatCode = "N2"
            Case 5
                loadedProgressText = MFC5LoadedProgressText
                stringFormatCode = "F3"
            Case 6
                loadedProgressText = MFC6LoadedProgressText
                stringFormatCode = "F3"
        End Select
    ...
    Private Sub Form1_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        End If
    End Sub
    Private Sub has3Axis()
        Dim ResponseLen As Integer
        Dim StrVar As String
        WriteCommand("$22000D%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            b_IsStringANumber(StrVar, st_IntChars, "$22%")
            st_has3AxisBoard = StrVar
        End If
    End Sub
    ...
    Private Sub MFC_1_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_1_Recipe_Button.Click
        ...
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            ...
            MFC(1).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(1).Text = DoubVal.ToString("N2")
            MFC(1).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub MFC_2_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_2_Recipe_Button.Click
        ...
        StrVar = InputBox("Format xx.yy (max value: " & range.ToString("N2") & ")", "MFC_2 Enter Flow Value", "")
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            If StrVar = "" Or StrVar.Length > 6 Then Return
            DoubVal = Convert.ToDouble(StrVar) 'Convert the string value to a floating point
            If DoubVal > MFC(2).GetRange() Or DoubVal < 0 Then Return
            SetGUILoadProgressBars(2, DoubVal)
            MFC(2).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(2).Text = DoubVal.ToString("N2")
            MFC(2).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub MFC_3_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_3_Recipe_Button.Click
        ...
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            ...
            MFC(3).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(3).Text = DoubVal.ToString("N2")
            MFC(3).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub MFC_4_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_4_Recipe_Button.Click
        ...
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            ...
            MFC(4).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(4).Text = DoubVal.ToString("N2")
            MFC(4).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub MFC_5_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_5_Recipe_Button.Click
        ...
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            ...
            MFC(5).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(5).Text = DoubVal.ToString("F3")
            MFC(5).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub MFC_6_Text_In_Button_Click(sender As Object, e As EventArgs) Handles Set_MFC_6_Recipe_Button.Click
        ...
        If b_IsStringValid(StrVar, st_DoubleChars, "Invalid Entry") Then
            ...
            MFC(6).SetLoadedFlow(DoubVal)
            MFCRecipeFlow(6).Text = DoubVal.ToString("F3")
            MFC(6).loadNewValue = True
        Else
            Return
        End If
    ...
    Private Sub RunStartUp()
        ...
        GetMFC5Range()
        GetMFC6Range()
        SetGUILoadedProgressNumbers(1)
        SetGUILoadedProgressNumbers(2)
        SetGUILoadedProgressNumbers(3)
        SetGUILoadedProgressNumbers(4)
        SetGUILoadedProgressNumbers(5)
        SetGUILoadedProgressNumbers(6)
        GetMFCRecipesFromController()
        GetRecipeMBStartPos()
        GetRecipeRFPower()
        GetMaxRFPower()
        GetAutoTuneSetting()
        ...
        Operator_Mode()
    End Sub
    Private Sub turnOn3AxisGUIElements()
        Label4.Visible = True
        CurrentStepTxtBox.Visible = True
        NextStepTxtBox.Visible = True

        RecipeThicknessTxt.Visible = True
        RecipeGapTxt.Visible = True
        RecipeOverLapTxt.Visible = True
        RecipeSpeedTxt.Visible = True
        RecipeCyclesTxt.Visible = True

        SetThicknessBtn.Visible = True
        SetGapBtn.Visible = True
        SetOverlapBtn.Visible = True
        SetSpeedBtn.Visible = True
        SetCyclesBtn.Visible = True

        Label22.Visible = True
        Label23.Visible = True
        Label24.Visible = True
        AxesXActual.Visible = True
        AxesYActual.Visible = True
        AxesZActual.Visible = True
        Label33.Visible = True
        Label34.Visible = True
        Label29.Visible = True

        RecipeYMaxTxt.Visible = True
        RecipeYMinTxt.Visible = True
        RecipeXMaxTxt.Visible = True
        RecipeXMinTxt.Visible = True

        SetYMaxBtn.Visible = True
        SetYMinBtn.Visible = True
        SetXMaxBtn.Visible = True
        SetXMinBtn.Visible = True

        ControllerLabel.Visible = True
        contollerONSquare.Visible = True

        LaserSenseSquare.Visible = True
        Label17.Visible = True

        PurgeOnLabel.Visible = True
        N2Purgelabel.Visible = True

        ChuckVacSquare.Visible = True
        Label18.Visible = True

        SetTwoSpotBtn.Visible = True
        SetDiameterBtn.Visible = True
        InitAxesBtn.Visible = True
        ...
        N2PurgeSquare.Visible = True
        RunScanBtn.Visible = True
        LabelLock.Visible = True

        Label3.Visible = True
        Line_Label2.Visible = True
        Line_Label4.Visible = True
        Label6.Visible = True
    ...
    Private Sub isControllerConnected()
        ...
        End If
    End Sub
    Private Sub GetCTLFirmwareVersion()
        Dim ResponseLen As Integer
        Dim StrVar As String
        ...
        End If
    End Sub
    Private Sub HowManyMFCs()
        Dim ResponseLen As Integer
        Dim StrVar As String
        'How many MFCs?
        WriteCommand("$220009%", 8) 'GET Number of MFCs (1-6) $2Axxx% xxxx = any length index number =>resp [!2Axxx;vv..vv#] vv..vv = value
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
    ...
    Private Sub GetMFC1Range()
        ...
        If ResponseLen > 8 Then
            ...
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(1).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFC2Range()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim DblVar As Double
        WriteCommand("$22002E%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(2).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFC3Range()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim DblVar As Double
        WriteCommand("$22002F%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(3).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFC4Range()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim DblVar As Double
        WriteCommand("$220030%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(4).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFC5Range()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim DblVar As Double
        WriteCommand("$220031%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(5).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFC6Range()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim DblVar As Double
        WriteCommand("$220032%", 8)
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(StrVar) Then
                DblVar = CDbl(StrVar)
                MFC(6).SetRange(DblVar)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub GetMFCRecipesFromController()
        Dim ResponseLen As Integer
        'The absolute Recipe addresses of 1-6
        Dim mfc As String() = {"$220036%", "$220037%", "$220038%", "$220039%", "$22003A%", "$22003B%"}
        For i = 1 To mfc.Length()
            WriteCommand(mfc(i - 1), 8)
            ResponseLen = ReadResponse(0)
            ExtractMFCRecipeFromResponse(i, ResponseLen)
        Next
    End Sub
    Private Sub ExtractMFCRecipeFromResponse(mfcNumber As Integer, responseLen As Integer)
        Dim controllerResponse As String
        Dim recipeSetpoint As Double
        If responseLen > 8 Then
            controllerResponse = st_RCV.Substring(7)
            controllerResponse = controllerResponse.Trim(New Char() {"#"c}) 'remove the last char
            If IsNumeric(controllerResponse) Then
                recipeSetpoint = CDbl(controllerResponse)
                SetMFCRecipe(mfcNumber, recipeSetpoint)
            Else
                MsgBox("NO RS485, unable to get MFC Range: check MFC connections")
            End If
        End If
    End Sub
    Private Sub SetMFCRecipe(mfcNumber As Integer, flow As Double)
        MFC(mfcNumber).SetLoadedFlow(flow)
        SetGUIRecipeMFCText(mfcNumber)
        SetGUILoadProgressBars(mfcNumber, flow.ToString())
    End Sub
    Private Sub SetGUIRecipeMFCText(mfcNumber As Integer)
        If mfcNumber = 5 Or mfcNumber = 6 Then
            MFCRecipeFlow(mfcNumber).Text = MFC(mfcNumber).GetLoadedValue().ToString("F3")
        Else
            MFCRecipeFlow(mfcNumber).Text = MFC(mfcNumber).GetLoadedValue().ToString("N2")
        End If
    End Sub

    Private Sub GetRecipeMBStartPos()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim IntVar As Integer
        WriteCommand("$22003D%", 8) 'GET RECIPE MB Start Position () $2Axxx% xxxx = any length index number =>resp [!2Axxx;vv..vv#] vv..vv = value
        ResponseLen = ReadResponse(0)
        If ResponseLen > 8 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            b_IsStringANumber(StrVar, st_DoubleChars, "$22%")
            IntVar = CInt(StrVar)
        ...
        End If
    End Sub
    Private Sub GetRecipeRFPower()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim IntVar As Integer
        WriteCommand("$22003C%", 8) 'GET RECIPE RF PWR Setpoint (Watts) $2Axxx% xxxx = any length index number =>resp [!2Axxx;vv..vv#] vv..vv = value        
        ResponseLen = ReadResponse(0)
        If ResponseLen > 7 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            b_IsStringANumber(StrVar, st_DoubleChars, "$22%")
            IntVar = CInt(StrVar)
            RF.LoadedSetPoint = Convert.ToInt32(IntVar, 10)
            RecipeWattsTxt.Text = CStr(RF.LoadedSetPoint) 'this is for the New GUI to view Loaded values
        End If
    End Sub
    Private Sub GetMaxRFPower()
        Dim ResponseLen As Integer
        Dim StrVar As String
        WriteCommand("$220017%", 8)  'Get Max RF power forward  $2Axxx% xxxx = any length index number =>resp [!2Axxx;vv..vv#] vv..vv = value
        ResponseLen = ReadResponse(0)
        If ResponseLen > 7 Then
            StrVar = st_RCV.Substring(7)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
        ...
        End If
    End Sub
    Private Sub GetAutoTuneSetting()
        Dim ResponseLen As Integer
        Dim StrVar As String
        Dim IntVar As Integer
        ...
        End If
    End Sub
    Private Sub ExecRecipe(status As String)
        Dim ResponseLen As Integer
        WriteCommand("$870" & status & "%", 6) 'SET_EXEC_RECIPE  $870p% p=1 Execute Recipe, p=0 RF off, Recipe off
        ResponseLen = ReadResponse(0)
    End Sub
    ...
    Private Sub RunPolling() 'Poll the Control and Axis PCBs for status (every 1 second)
        ...
        If CTL.CheckForPlasmaActivation() And b_autoScanActive = True And b_toggleAutoScan = True Then
        ...
        If CTL.CheckForAbortActivation() Then
            PublishAbortCode()
        End If

        If CTL.CheckForEStopActivation() Then
            WriteLogLine("ESTOP occurred, application shutting down.")
            Application.Exit()
        End If
    End Sub
    ...
    Public Sub ParseAbortCode(length As Integer)
        ...
        If length > 7 Then
            StrVar = st_RCV.Substring(3)
            StrVar = StrVar.Trim(New Char() {"#"c}) 'remove the last char
            b_IsStringANumber(StrVar, st_HexChars, "$8B%")
            IntVar = Convert.ToInt32(StrVar, 16)
            If AbortCodeMessages.ContainsKey(IntVar) Then
                errorMessage = AbortCodeMessages(IntVar)
    ...
    Public Sub SetGUILoadProgressBars(index As Integer, value As String)
        ...
        Dim range As Double = MFC(index).GetRange()
        Dim percent As Integer = CInt(Flow / range * 100)
    ...
    Private Sub RunCheckInput()
        ...
        End If
    End Sub
    Private Sub SendNewRecipeFlow(index As Integer)
        Dim recipeFlow As Double = MFC(index).GetLoadedValue()
        Dim recipeFlowCommand As String = "$410" & index.ToString() & recipeFlow.ToString("F3") & "%"
        Dim ResponseLen As Integer
        If MFC(index).isNewLoadValue() Then
            If index = 5 Or index = 6 Then
                MFCRecipeFlow(index).text = recipeFlow.ToString("F3")
            Else
                MFCRecipeFlow(index).text = recipeFlow.ToString("N2")
            End If
            WriteCommand(recipeFlowCommand, Len(recipeFlowCommand))  'SET_RCP_MFC_FLOW   $410mxxx.yy% 1<=m<=4, xxx.yy = flow rate; resp[!410mxxx.yy#]
            ResponseLen = ReadResponse(0)
            MFC(index).loadNewValue = False
        End If
    End Sub
    ...
    Private Sub GetExeCfg()
        ...
        For Ctr As Integer = 1 To ExeConfigLines.Length - 1
            ...
            Select Case ExeConfigParamName 'load up the Config values
                ...
                Case "HANDSHAKE"
                    Exe_Cfg.HANDSHAKE = ExeConfigParamValue
                    st_hasHandshake = Exe_Cfg.HANDSHAKE
                Case "PW"
                    Exe_Cfg.Pw = ExeConfigParamValue
                    st_password = Exe_Cfg.Pw
    ...
    End Sub
    Private Sub ToggleLEDGUIDisplayStatus(sender As Object, e As EventArgs) Handles ControllerStatusLEDSToolStripMenuItem.Click
        'All recipe settings, except for Batchlogging which is set on the tool.
    ...
    Public Sub LoadRecipeValues()
        ...
        For Ctr As Integer = 1 To st_RecipeLines.Length - 1
            ...
            Select Case st_RecipeParamName 'load up the recipe values
                Case "MFC1"
                    SetMFCRecipe(1, CDbl(st_RecipeParamValue))
                Case "MFC2"
                    SetMFCRecipe(2, CDbl(st_RecipeParamValue))
                Case "MFC3"
                    SetMFCRecipe(3, CDbl(st_RecipeParamValue))
                Case "MFC4"
                    SetMFCRecipe(4, CDbl(st_RecipeParamValue))
                Case "MFC5"
                    SetMFCRecipe(5, CDbl(st_RecipeParamValue))
                Case "MFC6"
                    SetMFCRecipe(6, CDbl(st_RecipeParamValue))
                Case "PWR"
        ...
        MFC(1).loadNewValue = True 'signal main loop to load the new flow
        MFC(2).loadNewValue = True
        MFC(3).loadNewValue = True
        MFC(4).loadNewValue = True
        MFC(5).loadNewValue = True
